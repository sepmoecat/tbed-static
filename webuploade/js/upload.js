...
(function( $ ){
    ...
    $(function() {
        ...
        // 实例化
        if(VisitorUpload==1 && userclose==1){
            ...
            uploader.on( 'uploadSuccess', function(file,response) {
                ...
                if(response.code=='1000'){
                    layui.use('layer', function () {
                        layer = layui.layer;
                        layer.msg('上传失败，管理员已经关闭你的上传功能。', {icon: 2});
                    });
                }else if(response.code=='4000'){
                    layui.use('layer', function () {
                        layer = layui.layer;
                        layer.msg("非法文件上传，请上传图片格式文件。", {icon: 2});
                    });
                    ...
                    if(response.data!=null){
                        ...
                        if(f==0){
                            setTimeout(function(){
                                layer.tips('这里可以生成画廊哦', '.el-backtop', {fixed:true,tips: [4, '#666666']})
                            }, 5000);
                            f=1;
                        }
                    }
            ...
            }else if(VisitorUpload==1 && userclose!=1){
                $('#urlsc').html('<a id="jyupdate">已禁止用户上传功能</a>');
            }else{
                $('#urlsc').html('<a id="jyupdate">管理员已禁止所有人的上传功能</a>');
            }
            return;
        }
